/*
 * File: app/view/adminGeneral/RolesUsuarioViewController.js
 *
...
Ext.define('Tramites.view.adminGeneral.RolesUsuarioViewController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.admingeneral.rolesusuario',

    onCbUsuarioBuscarKeyup: function(textfield, e, eOpts) {
        //función para que se presenten usuarios similares a los que el usuario va escribiendo
    ...
    onBtnLimpiarUsuarioClick: function(button, e, eOpts) {
        ...
        strUsuario.proxy.extraParams={
            usr: 0//trae los usuarios activos
        };
        strUsuario.load();
        Ext.getCmp('cbParamRol').reset();
        Ext.getCmp('grdRol').getStore().removeAll();
        Ext.getCmp('btnAgregarRol').setHidden(true);
        Ext.getCmp('btnQuitarRol').setHidden(true);
    },
    ...
    onCbParamRolExpand: function(field, eOpts) {
        var storeParamRol = Ext.getCmp('cbParamRol').getStore();
        ...
        {
            storeParamRol.proxy.extraParams={
                param:2//obtiene todos los parámetros
            };
            storeParamRol.load();
        }
    },

    onCbParamRolSelect: function(combo, record, eOpts) {
        if(Ext.getCmp('cbParamRol').getValue !== null){
            var g = Ext.getCmp('grdUsuarioRol');
            var rec = g.selection;
            ...
            strRoles.proxy.extraParams = {
                ...
                parametro: Ext.getCmp('cbParamRol').getValue()
            };
            strRoles.load();
            Ext.getCmp('btnAgregarRol').setHidden(false);
            Ext.getCmp('btnQuitarRol').setHidden(false);
        }
    },
    ...
    onBtnAgregarRolClick: function(button, e, eOpts) {
        ...
        else
        {
            ...
            store.insert(0,//crea una nueva linea en la grid
            {
    ...
    onGrdRolCellDblClick: function(tableview, td, cellIndex, record, tr, rowIndex, e, eOpts) {
        if(record.data.ROL_ID === 0)//al doble click se habilita el combobox
