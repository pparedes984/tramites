...
Ext.define('Tramites.view.winNuevoTramiteViewController', {
    ...
    onCbAsunto2Select: function(combo, record, eOpts) {
        if(Ext.getCmp('cbAsunto2').getValue()==2)
        {
            Ext.getCmp('dtFechaFinViajeViatico').allowBlank = false;
            Ext.getCmp('dtFechaFinViajeViatico').focus(true);
        }
        else
        Ext.getCmp('dtFechaFinViajeViatico').allowBlank = true;

        //Campos opcionales y obligatorios
        if(Ext.getCmp('cbAsunto2').getValue()==22){
            Ext.Msg.alert('Atención','Ingresar trámite por SAP');
            Ext.getCmp('cbAsunto2').clearValue();
        }
        else{
            ...
            Ext.Ajax.request({
                ...
                success: function (response, options) {
                    ...
                    Ext.getCmp('txtTelefono').allowBlank = false;
                    Ext.getCmp('txtTelefono').focus(true);
                    ...
                    if(viaticos !== undefined){//Identifica si Viaticos son obligatorios
                        ...
                        Ext.getCmp('dtFechaFinViajeViatico').focus(true);
                        Ext.getCmp('dtFechaFinViajeViatico').focus(false);
    ...
    onSave: function(button, e, eOpts) {
        /*var me = this;
        var form = Ext.getCmp('frmTramite').getForm(),

        store = Ext.getStore('Registro');
...
var me = this;
var store = Ext.getStore('Registro');
var forma = Ext.getCmp('frmTramite').getForm();
if (forma.isValid()) {
var rec  = Ext.getCmp('frmTramite').getForm().getValues();
var resp    = forma.getValues();
if(resp.TRA_FECHA_VIAJE_VIATICO ===  '')//borra variables para que no se llenen con una fecha erronea cuando no se ha selecionado una fecha
{
    delete resp.TRA_FECHA_VIAJE_VIATICO;
}
if(resp.TRA_FECHA_VIAJE_FIN_VIATICO ===  '')
{
    delete resp.TRA_FECHA_VIAJE_FIN_VIATICO;
}
resp.TRA_ANULADO = 0;
resp.TRA_RECIBIDO = 0;
resp.TRA_PRM_ID = Ext.getCmp('cbParametroPrincipal').getValue();

Ext.Ajax.request({
    url: '../servidor/tramites/registro/save',
    method:"post",
    params: {
        idParametro: Ext.getCmp('cbParametroPrincipal').getValue(),//Se envia el codigo del parametro,
        data: Ext.encode(resp)
    },
    success: function(batch, options)
    {
...
Ext.getCmp('txtFormExt').setDisabled(false);
    ...
    onEditarClick: function(button, e, eOpts) {
        /*var me = this;
        var forma = Ext.getCmp('frmTramite').getForm();
        if (forma.isValid()) {
        var store = Ext.getStore('Registro');
...
if (forma.isValid()) {
var rec  = Ext.getCmp('frmTramite').getForm().getValues();
var resp    = forma.getRecord();
if(Ext.getCmp('pnlViaticos').isVisible() === false)//borra variables para que no se llenen con una fecha erronea cuando no se ha selecionado una fecha
{
    delete rec.TRA_FECHA_VIAJE_VIATICO;
    delete rec.TRA_FECHA_VIAJE_FIN_VIATICO;
}
if(rec.TRA_FECHA_VIAJE_VIATICO ===  '')
{
    delete rec.TRA_FECHA_VIAJE_VIATICO;
}
if(rec.TRA_FECHA_VIAJE_FIN_VIATICO ===  '')
{
    delete rec.TRA_FECHA_VIAJE_FIN_VIATICO;
}
rec.TRA_ANULADO = 0;
rec.TRA_PRM_ID = resp.data.TRA_PRM_ID;
rec.TRA_RECIBIDO = resp.data.TRA_RECIBIDO;
rec.TRA_CODIGO = resp.data.TRA_CODIGO;

Ext.Ajax.request({
    url: '../servidor/tramites/registro/update',
    method:"post",
    params: {
        idParametro: Ext.getCmp('cbParametroPrincipal').getValue(),
        data: Ext.encode(rec)
...
Ext.getCmp('txtFormExt').setDisabled(false);
    ...
    onWinNuevoTramiteAfterRender: function(component, eOpts) {
        ...
        if(component.curRec){ //Actualizar tramite
            ...
            fecha = component.curRec.data.TRA_FECHA;
            fecha = fecha.slice(0, -4);
            Ext.getCmp('dtFechaFinViajeViatico').minValue = new Date();
            Ext.getCmp('dtFecha').setValue(fecha);
            console.log(component.curRec);
        }
        ...
        else//Nuevo tramite
        {
            ...
            Ext.getCmp('dtFecha').setValue(new Date());
            Ext.getCmp('dtFechaFinViajeViatico').minValue = new Date();
            Ext.getCmp('txtMoneda').setValue('DOLAR');
        }
    }
