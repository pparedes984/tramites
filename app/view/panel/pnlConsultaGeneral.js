/*
 * File: app/view/panel/pnlConsultaGeneral.js
 *
 * This file was generated by Sencha Architect version 4.2.4.
 * http://www.sencha.com/products/architect/
 *
 * This file requires use of the Ext JS 6.5.x Classic library, under independent license.
 * License of Sencha Architect does not include license for Ext JS 6.5.x Classic. For more
 * details see http://www.sencha.com/license or contact license@sencha.com.
 *
 * This file will be auto-generated each and everytime you save your project.
 *
 * Do NOT hand edit this file.
 */

Ext.define('Tramites.view.panel.pnlConsultaGeneral', {
    extend: 'Ext.panel.Panel',
    alias: 'widget.panel.pnlconsultageneral',

    requires: [
        'Tramites.view.panel.pnlNuevoViewModel5',
        'Tramites.view.panel.pnlNuevoViewController5',
        'Ext.grid.Panel',
        'Ext.view.Table',
        'Ext.grid.column.Number',
        'Ext.grid.column.Date',
        'Ext.grid.column.Check',
        'Ext.toolbar.Toolbar',
        'Ext.button.Button'
    ],

    controller: 'panel.pnlconsultageneral',
    viewModel: {
        type: 'panel.pnlconsultageneral'
    },
    title: 'Consulta general de Trámites',

    layout: {
        type: 'hbox',
        align: 'stretch'
    },
    items: [
        {
            xtype: 'panel',
            flex: 1,
            scrollable: true,
            layout: 'border',
            items: [
                {
                    xtype: 'gridpanel',
                    region: 'center',
                    cls: 'myCls',
                    id: 'grdConsulta2',
                    title: 'Trámites',
                    forceFit: true,
                    store: 'Consulta1',
                    viewConfig: {
                        listeners: {
                            celldblclick: 'onTableCellDblClick',
                            cellclick: 'onTableCellClick'
                        }
                    },
                    columns: [
                        {
                            xtype: 'numbercolumn',
                            width: 119,
                            dataIndex: 'TRA_CODIGO',
                            text: 'Número de trámite',
                            format: '0'
                        },
                        {
                            xtype: 'datecolumn',
                            dataIndex: 'TRA_FECHA',
                            text: 'Fecha de creación',
                            format: 'Y-m-d H:i:s'
                        },
                        {
                            xtype: 'datecolumn',
                            dataIndex: 'TRA_FECHARECIBIDO',
                            text: 'Fecha de recepción',
                            format: 'Y-m-d H:i:s'
                        },
                        {
                            xtype: 'gridcolumn',
                            hidden: true,
                            dataIndex: 'PRM_NOMBRE',
                            text: 'Parámetro'
                        },
                        {
                            xtype: 'gridcolumn',
                            dataIndex: 'ASU_DESCRIPCION',
                            text: 'Asunto'
                        },
                        {
                            xtype: 'gridcolumn',
                            dataIndex: 'UND_NOMBRE',
                            text: 'Unidad Solicitante'
                        },
                        {
                            xtype: 'checkcolumn',
                            disabled: true,
                            hidden: true,
                            dataIndex: 'TRA_RECIBIDO',
                            text: 'Recibido (DGF)'
                        },
                        {
                            xtype: 'gridcolumn',
                            dataIndex: 'TRA_GASTO',
                            text: 'Und. de Gasto'
                        },
                        {
                            xtype: 'gridcolumn',
                            hidden: true,
                            dataIndex: 'TRA_OFICIO',
                            text: 'N° Oficio'
                        },
                        {
                            xtype: 'gridcolumn',
                            dataIndex: 'TRA_BENEFICIARIO1',
                            text: 'Beneficiario'
                        },
                        {
                            xtype: 'gridcolumn',
                            hidden: true,
                            dataIndex: 'TRA_BENEFICIARIO2',
                            text: '2do Beneficiario'
                        },
                        {
                            xtype: 'gridcolumn',
                            dataIndex: 'TRA_PROYECTO',
                            text: 'Proyecto/Partida'
                        },
                        {
                            xtype: 'gridcolumn',
                            dataIndex: 'TRA_DESCRIPCION',
                            text: 'Descripcion'
                        },
                        {
                            xtype: 'checkcolumn',
                            disabled: true,
                            dataIndex: 'TRA_ANULADO',
                            text: 'Anulado'
                        }
                    ],
                    listeners: {
                        render: 'onGrdConsulta1Render'
                    },
                    dockedItems: [
                        {
                            xtype: 'toolbar',
                            dock: 'top',
                            items: [
                                {
                                    xtype: 'button',
                                    iconAlign: 'right',
                                    iconCls: 'x-fa fa-search',
                                    text: 'Buscar',
                                    listeners: {
                                        click: 'onButtonClick'
                                    }
                                },
                                {
                                    xtype: 'button',
                                    disabled: true,
                                    id: 'btnEstadosConsultar1',
                                    text: 'Ver Estados',
                                    listeners: {
                                        click: 'onBtnEstadosConsultarClick'
                                    }
                                }
                            ]
                        }
                    ]
                }
            ]
        }
    ]

});